...
def gen_expr(ctx, result, op):
    ...
    if isinstance(op, ast.Subscript):
        irlib.assertion(
            op.value.id in ctx.func_locals, f"undefined variable {op.value.id}"
        )
        regn_src = ctx.func_locals[op.value.id]
        regn_offset = ctx.alloc_vreg()
        return gen_expr(ctx, regn_offset, op.slice) + [
            ir.ReadPointerReg(regn_src, result, regn_offset)
        ]
    ...
    return (
        ops[0][0]
        + ops[1][0]
        + [ir.ThreeAddressInstr(result, ops[0][1], ops[1][1], ir.astOp2IrOp(op.op))]
    )
