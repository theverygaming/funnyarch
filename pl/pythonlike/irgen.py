...
def gen_ast_node(node, depth = 0):
    ...
    if isinstance(node, ast.Assign):
        ...
        if depth == 0: # TODO: better way to check depth
            _assert_instance(node.value, ast.Constant, "can only assign constant to globals")
        else:
            _assertion(False, "local variables unsupported")
    ...
    if isinstance(node, ast.FunctionDef) and depth == 0: # TODO: better way to check depth
        ...
        irl = []
        for n in node.body:
            irl += gen_ast_node(n, depth+1)
        irl = [ir.FuncBegin(node.name, _func_is_leaf, _func_nlocals)] + irl + [ir.FuncEnd(node.name, _func_is_leaf, _func_nlocals)]
        # FIXME: patch ir.FuncCall's with correct nlocals
        return irl
